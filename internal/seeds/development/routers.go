...
func (s *MikrotikSeeder) Name() string {
	return "Development MikroTik Routers"
}
...
func (s *MikrotikSeeder) Seed(db *gorm.DB) error {
	...
	devRouters := []*model.MikrotikRouter{
		{
			ID:       uuid.MustParse("440e8400-e29b-41d4-a716-446655440001"),
			Name:     "Dev Router 1",
	...
	for _, router := range devRouters {
		...
		if result.Error == gorm.ErrRecordNotFound {
			if err := db.Create(router).Error; err != nil {
				return fmt.Errorf("failed to create dev router %s: %w", router.Name, err)
			}
			fmt.Printf("[MIKROTIK DEV SEED] Created: %s\n", router.Name)
		} else if result.Error != nil {
			return fmt.Errorf("failed to check existing router %s: %w", router.Name, result.Error)
		} else {
			fmt.Printf("[MIKROTIK DEV SEED] Already exists: %s\n", router.Name)
		}
	}
