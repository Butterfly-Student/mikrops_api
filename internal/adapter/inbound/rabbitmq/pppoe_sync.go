...
func (c *PppoeSyncConsumer) HandleCreate(msg []byte) bool {
	...
	// Verify PPPoE account exists (account is used in logSync functions)
	_, err = c.domain.PppoeAccount().FindByID(ctx, message.PppoeAccountID)
	if err != nil {
	...
	err = c.domain.Mikrotik().CreatePPPoESecret(ctx, message.NasID,
...
func (c *PppoeSyncConsumer) HandleIsolate(msg []byte) bool {
	...
	err = c.domain.Mikrotik().UpdatePPPoEProfile(ctx, message.NasID,
	...
	_ = c.domain.Mikrotik().DisconnectPPPoESession(ctx, message.NasID, message.Username)
...
func (c *PppoeSyncConsumer) logSyncError(ctx context.Context, pppoeAccountID, nasID, action, errorMsg string) {
	...
	if err != nil {
		...
		logInput := model.MikrotikSyncLogInput{
			...
			Action:             action,
			ResourceType:       "ppp_secret",
			ResourceIdentifier: pppoeAccountID,
